              ...
              children: [
                      ...
                      children: [
                        DropdownButtonHideUnderline(
                          child: DropdownButton<String>(
                            ...
                            style: TextStyle(color: isDark ? Colors.white : Colors.black87, fontSize: 16, fontWeight: FontWeight.w600, fontFamily: 'Poppins'),
                        ...
                        Text('Your CGPA', style: TextStyle(fontSize: 16, color: isDark ? Colors.white70 : Colors.black54)),
                        Text(_cgpa.toStringAsFixed(2), style: TextStyle(fontSize: 56, fontWeight: FontWeight.bold, color: isDark ? Colors.white : primaryColor)),
                      ],
                    ),
                  ),
                ),
                if (_history.isNotEmpty)
                  Container(
                    ...
                    child: ListView(
                      children: _history.map((h) => Text('History: $h', style: TextStyle(color: isDark ? Colors.white54 : Colors.black45, fontSize: 12), textAlign: TextAlign.center)).toList(),
                    ),
                  ),
                Expanded(
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) => _buildSubjectRow(_subjects[index], index, isDark, primaryColor),
                  ),
                ),
                Padding(
  ...
  Widget _buildSubjectRow(Subject subject, int index, bool isDark, Color primaryColor) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(color: primaryColor.withOpacity(isDark ? 0.1 : 0.05), blurRadius: 10, offset: const Offset(0, 4))
        ]
      ),
      child: Row(
        children: [
          Container(
            ...
            child: Text('Sub ${index + 1}', style: TextStyle(fontWeight: FontWeight.bold, color: isDark ? Colors.white : primaryColor)),
          ),
          const SizedBox(width: 16),
          Expanded(
            child: TextFormField(
          ...
          Expanded(
            child: DropdownButtonFormField<double>(
              ...
              items: _gradingSystem == '10-Point Scale' ? const [
              ...
              onChanged: (val) { if (val != null) subject.gradePoint = val; },
            ),
          ),
          IconButton(icon: const Icon(Icons.delete_outline, color: Colors.redAccent), onPressed: () => setState(() => _subjects.removeAt(index)))
        ],
      ),
    );
  }
